...
@section('title', 'Simulasi KPR BNI – Bintaro Land Property')
...
<div x-data="kprCalculator" x-cloak>
  ...
  {{-- ── RUNNING TEXT PROMO ── --}}
  @if(isset($kprPromos) && $kprPromos->count() > 0)
  <div class="kpr-marquee-wrap">
    <div class="kpr-marquee-content">
        @foreach($kprPromos as $promo)
            <span class="kpr-marquee-item">
                ...
                Info Promo Terbaru: <strong>{{ $promo->nama }}</strong> — Bunga Fix <strong>{{ $promo->bunga_fix }}%</strong> selama {{ $promo->masa_fix }} Tahun, dilanjutkan Floating {{ $promo->bunga_floating }}%. Segera konsultasikan KPR Anda!
            </span>
        @endforeach
    </div>
  </div>
      ...
      <div class="kpr-card kpr-result-card">
        ...
        <div class="kpr-result-body">
          ...
          <div class="kpr-action-row">
            <a :href="`https://wa.me/{{ env('WHATSAPP_NUMBER', '6281234567890') }}?text=Halo+saya+ingin+tanya+KPR+BNI+untuk+properti+senilai+${formatRp(hargaRaw)}`"

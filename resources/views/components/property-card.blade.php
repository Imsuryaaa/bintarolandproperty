    ...
    <div class="relative overflow-hidden aspect-[4/3]">
        ...
        <div class="absolute top-2 left-2 sm:top-3 sm:left-3 flex flex-wrap items-center gap-1 sm:gap-1.5 z-20 max-w-[90%]">
            ...
            @if($property->property_condition)
                ...
                @endphp
                <span class="inline-flex items-center px-1.5 py-0.5 sm:px-2 sm:py-1 text-[8px] sm:text-[10px] uppercase tracking-wider font-bold text-white rounded {{ $colorClass }} backdrop-blur-sm shadow-sm">
                    {{ $label }}
                </span>
            @endif
    ...
    <div class="p-2 sm:p-4">
        ...
        @if($property->categories->count())
            <div class="flex flex-wrap gap-1 mb-1.5 sm:mb-2">
                @foreach($property->categories as $cat)
                    <span class="badge-gold text-[9px] sm:text-[11px] px-1.5 py-0.5 sm:px-2 sm:py-0.5">{{ $cat->name }}</span>
                @endforeach
            </div>
        @endif
        ...
        @if($property->city || $property->district)
            <p class="flex items-center gap-1 text-[10px] sm:text-xs text-gray-500 dark:text-charcoal-400 mb-2 sm:mb-3">
                ...
                <span class="truncate">{{ $property->location_label }}</span>
            </p>
        @endif
        ...
        <div class="flex flex-wrap items-center gap-1.5 sm:gap-3 text-[10px] sm:text-xs text-gray-500 dark:text-charcoal-400 pt-2 sm:pt-3 border-t border-gray-100 dark:border-charcoal-800">
            ...
            <span class="flex items-center gap-0.5 sm:gap-1">
                ...
                LT {{ number_format($property->land_area, 0, ',', '.') }}
            </span>
            <span class="flex items-center gap-0.5 sm:gap-1">
                <svg class="w-3 h-3 sm:w-3.5 sm:h-3.5 text-brand-500" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/>
                </svg>
                LB {{ number_format($property->build_area, 0, ',', '.') }}
            </span>
        </div>

        {{-- CTA button (relative z-20 so it's clickable above the overlay link) --}}
        <a href="{{ route('property.show', $property->slug) }}"

...
<script type="application/ld+json">
{
  ...
  "@id": "{{ url('/') }}",
  "url": "{{ url('/') }}",
  "telephone": "{{ env('WHATSAPP_NUMBER', '6281234567890') }}",
  "address": {
    "@type": "PostalAddress",
...
<section id="search-section"
    ...
    <div class="container-main">
        ...
        {{-- ── Desktop Search / Filter (lg:block) ── --}}
        ...
        {{-- ── Mobile Search / Filter (lg:hidden) ── --}}
        <div class="lg:hidden py-3">
            <form action="{{ route('search') }}" method="POST" id="mobile-search-form" class="relative">
                @csrf
                ...
                <div class="flex overflow-x-auto no-scrollbar gap-2 mt-3 pb-1">
                    <a href="{{ route('home') }}" class="whitespace-nowrap px-4 py-1.5 rounded-full {{ !$hasCategory ? 'bg-brand-500 text-white border-brand-500' : 'bg-white dark:bg-charcoal-950 text-gray-600 dark:text-gray-300 border-gray-200 dark:border-charcoal-700' }} text-xs font-medium border">Semua</a>
                    @foreach($parentCategories ?? [] as $parent)
                        <a href="{{ route('home', ['category' => $parent->group_type]) }}" class="whitespace-nowrap px-4 py-1.5 rounded-full {{ $categorySlug === $parent->group_type ? 'bg-brand-500 text-white border-brand-500' : 'bg-white dark:bg-charcoal-950 text-gray-600 dark:text-gray-300 border-gray-200 dark:border-charcoal-700' }} text-xs font-medium border">{{ $parent->name }}</a>
                    @endforeach
                </div>
            </form>
        </div>
...
<section class="py-12 lg:py-16 bg-gray-50 dark:bg-charcoal-900/50">
    <div class="container-main">
        ...
        @php $allFeatured = $featuredProperties->all(); @endphp

        {{-- ── Pool: semua kartu disimpan di sini, JS yang mengatur slide ── --}}
        <div id="hotsale-pool" aria-hidden="true" style="display:none">
            @foreach($allFeatured as $property)
